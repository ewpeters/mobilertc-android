...
//        Log.i(TAG, "isSupportVirtualBackground: " + bgHelper.isSupportVirtualBackground());
    ...
    public void onClickSwitchShare(View view) {
        if (shareUserList.size() > 1) {
            ShareListDialog.show(this, shareUserList, this);
            return;
        }
    ...
    @Override
    public void onUserClicked(ZoomVideoSDKUser user) {
        subscribeShareByUser(user);
    }
...
//            }
    ...
    private void subscribeShareWhenStatusChange() {
        subscribeShareByUser(currentShareUser);
        updateVideoAvatar(true);
        selectAndScrollToUser(currentShareUser);
    ...
    public void onUserShareStatusChanged(ZoomVideoSDKShareHelper shareHelper, ZoomVideoSDKUser userInfo, ZoomVideoSDKShareStatus status) {
        ...
        if (status == ZoomVideoSDKShareStatus.ZoomVideoSDKShareStatus_Start) {
            if (currentShareUser != session.getMySelf()) {
                subscribeShareWhenStatusChange();
            } else {
